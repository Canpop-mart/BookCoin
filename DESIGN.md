...
**Non-goals**
- Not a fraud-proof anti-cheat system. It's family, so trust is the default; we add just enough friction that cheating feels pointless and obvious.
...
Because ranking is time-based, **comics/manga/webtoons and audiobooks all "just work"**, with no per-format fudge factor needed. (Audiobooks become first-class for free, since they have no pages.)
...
- **Personal monthly goals.** Everyone competes against their *own* target; "% of goal hit" is its own board. The wholesome version, where anyone can "win" by beating themselves.
...
### 8.5 Monthly themes
One shared goal everyone chips at, like "Nonfiction November" or "Spooky October."
...
A tiered prize catalog, from silly to meaningful:
- **Low tier (honor system):** silly/cheap perks like picking the next family movie or a "skip a chore" token.
...
- Genre taxonomy, whether a fixed list, free tags, or pulled from an integration.
